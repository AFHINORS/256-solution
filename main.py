...
def compress(src,dst):
 ...
 for (tag,n),plan in F.items():
  ...
  for j,code in enumerate(plan):
   ...
   z+=x
  add(out,z)
 rg=residual(cs,used);cm={1:(1,4),6:(6,7),12:(12,13)};skip={4,7,13};dl={2,5}
 for k,ids in enumerate(rg):
  if k in skip:continue
  ks=cm.get(k,(k,));b=b''.join(cs[i][3] for j in ks for i in rg[j])
  if cs[rg[k][0]][0]==b'LOGS' and cs[rg[k][0]][1][1] in dl:b=pred(b,0,1)
  add(out,b)
...
def decompress(src,dst):
 ...
 for k,ids in enumerate(rg):
  if k in skip:continue
  x,o=take(a,o)
  if cs[rg[k][0]][0]==b'LOGS' and cs[rg[k][0]][1][1] in dl:x=pred(x,1,1)
  q=0
  for j in cm.get(k,(k,)):
