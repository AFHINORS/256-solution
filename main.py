...
CODECS = ("lzma", "lc0", "zlib", "bz2", "lc1", "store", "lc2", "pb2")
ZB = {"id": 33, "preset": 9, "nice_len": 273, "dict_size": 1 << 26}
...
ZF2 = _z(2)
ZP2 = _z(0, 2)
ZFIL = [ZF, Z0, None, None, Z1, None, ZF2, ZP2]
...
FPLAN = {
    (b"IMG ", 655360): bytes.fromhex("222829292e2eff2929ff252dff2aff2e"),
    (b"DUP ", 174784): bytes.fromhex("ff2affffffffff26ffffff"),
    (b"WAVE", 131090): bytes.fromhex("ffffff212124ff2823"),
    (b"SEQ2", 174784): bytes.fromhex("22ff2421ffff"),
...
def compress(src, dst):
    ...
    packed = [None] * len(stream)
    for i, alt, sz in ((7, 6, 362971), (16, 7, 749952),
                       (18, 1, 4587520), (20, 1, 1310720)):
        if i < len(stream) and len(stream[i]) == sz:
            try:
                if packed[i] is None:
                    packed[i] = pack(stream[i], cc[i])
                y = pack(stream[i], alt)
                if len(y) < len(packed[i]):
                    cc[i] = alt
                    packed[i] = y
            except Exception:
                pass
    ...
    out.extend(bytes(cc))
    for i, (b, c) in enumerate(zip(stream, cc)):
        y = packed[i] if packed[i] is not None else pack(b, c)
